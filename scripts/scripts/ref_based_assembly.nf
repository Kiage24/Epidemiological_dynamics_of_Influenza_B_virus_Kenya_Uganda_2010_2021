...
params.fastqreads = "../data/concat_fastqs/*.fastq"
params.refgenome = "../data/run1/reference/*.fasta"
params.outdir = "../results/medaka"
...
process maptoref{
    ...
    process polish_consensus {
    publishDir "${params.outputDir}/medaka", mode:'copy'
    tag "Polishing of assembly consensus sequences"

    input:
    // Expecting assembled consensus sequences
    tuple path(porechopfastq), path(consensusfasta)
   
    output:
    path "${consensusfilename}.fasta"

    script:
    consensusfilename = consensusfasta.simpleName
    """
    medaka_consensus \
        -i $porechopfastq \
        -d $consensusfasta \
    ...
    """
     ...
     workflow{
        ...
        polish_consensus(sequences_fastq, consensus.out)

...
vic_gisaid <- subset(vic_gisaid, year >=2010)

yam_gisaid <- yam_gisaid %>% drop_na(collection_date)
yam_gisaid <- subset(yam_gisaid, year >=2010)
...
subsample <- function (subsample_continent, subsample_year, subsample_month) {
  gisaid1 <- filter(yam_gisaid, segment == "NA" & as.numeric(year) == subsample_year & continent == subsample_continent & as.numeric(month) == subsample_month)
  num_rows <- nrow(gisaid1)
  num_samples <- min(num_rows, 10)
  
  if (num_samples > 0) {
    data_subsample <- sample(gisaid1$isolate_id, num_samples)
    dataset_subsampled <- yam_gisaid[yam_gisaid$isolate_id %in% data_subsample, ]
  ...
  for (year in years) {
    for (month in months) {
      ...
      if (!is.null(subsample_data)) {
        ...
        file_name <- paste0("~/Desktop/Msc/data/gisaid/yam_subsample/", continent, "_", year, month, ".fasta")
